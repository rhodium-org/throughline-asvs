...
def short_title(text: str) -> str:
    ...
    the first ``[,;.]`` and produced labels like ``That`` and
    ``Data selection or database queries (e``. Most ASVS clauses are a single sentence,
    so most titles are the whole clause; that is the intended outcome, not an accident.
